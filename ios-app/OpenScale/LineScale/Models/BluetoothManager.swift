...
struct OpenScaleBLE {
    ...
    static let deviceNameCharUUID = CBUUID(string: "8A2C5F47-B91E-4D36-A6C8-9F0E7D3B1C28")
}
...
class BluetoothManager: NSObject, ObservableObject {
    ...
    private var openScalePeripheral: CBPeripheral?
    ...
    func startScanning() {
        ...
        centralManager.scanForPeripherals(
            withServices: [OpenScaleBLE.serviceUUID],
    ...
    func connect(to peripheral: CBPeripheral) {
        ...
        openScalePeripheral = peripheral
        centralManager.connect(peripheral, options: nil)
    }

    /// Disconnect from current device
    func disconnect() {
        if let peripheral = openScalePeripheral {
            centralManager.cancelPeripheralConnection(peripheral)
        }
        openScalePeripheral = nil
        connectionState = .disconnected
    }
    ...
    func tare() {
        guard let characteristic = tareCharacteristic,
              let peripheral = openScalePeripheral else {
    ...
    func readDeviceName() {
        guard let characteristic = deviceNameCharacteristic,
              let peripheral = openScalePeripheral else {
            print("Cannot read device name: not connected")
            return
        }
    ...
    /// Set the device name (max 20 characters)
    func setDeviceName(_ name: String) {
        guard let characteristic = deviceNameCharacteristic,
              let peripheral = openScalePeripheral else {
            print("Cannot set device name: not connected")
            return
        }

        let trimmedName = String(name.prefix(20))
        guard let data = trimmedName.data(using: .utf8) else {
            print("Cannot encode device name")
            return
        }

        peripheral.writeValue(data, for: characteristic, type: .withResponse)
        deviceName = trimmedName
        print("Device name set to \(trimmedName)")
    }
...
extension BluetoothManager: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager,
        ...
        // This allows multiple OpenScale devices to coexist in the same room
    }
    ...
    func centralManager(_ central: CBCentralManager,
        ...
        peripheral.discoverServices([OpenScaleBLE.serviceUUID])
    }
...
extension BluetoothManager: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral,
        ...
        for service in services {
            if service.uuid == OpenScaleBLE.serviceUUID {
                print("Found OpenScale service")
