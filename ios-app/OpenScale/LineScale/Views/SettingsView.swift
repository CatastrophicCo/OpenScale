        ...
        NavigationStack {
            Form {
                ...
                Section("Device") {
                    ...
                    if bluetoothManager.connectionState == .connected {
                        HStack {
                            Text("Device Name")
                            Spacer()
                            Text(bluetoothManager.deviceName.isEmpty ? "Unknown" : bluetoothManager.deviceName)
                                .foregroundColor(.secondary)
                        }
                    }
                    ...
                    if bluetoothManager.connectionState == .disconnected {
                        ...
                        Button("Rename Device") {
                            newDeviceName = bluetoothManager.deviceName
                ...
                Section("About") {
                    ...
                    Link("GitHub Repository", destination: URL(string: "https://github.com/CatastrophicCo/OpenScale")!)

                    Link("Report Issue", destination: URL(string: "https://github.com/CatastrophicCo/OpenScale/issues")!)
                }
            }
            ...
            .alert("Rename Device", isPresented: $showRenameAlert) {
                ...
                Button("Rename") {
                    if !newDeviceName.isEmpty {
                        bluetoothManager.setDeviceName(newDeviceName)
                    }
                }
            } message: {
                Text("Enter a new name for your device (max 20 characters). Restart the device for the new name to appear in Bluetooth scanning.")
            }
        }
    }
            ...
            VStack {
                ...
                if bluetoothManager.discoveredDevices.isEmpty && bluetoothManager.connectionState != .scanning {
                    VStack(spacing: 16) {
                        ...
                        Text("Make sure your OpenScale is powered on and nearby")
                            .font(.subheadline)
                            .foregroundColor(.secondary)
                    ...
                    Text("Each OpenScale has a unique name (e.g., OpenScale-A1B2). Select your device from the list above.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
                        .padding()
